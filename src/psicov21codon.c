...
/* Convert a codon of three letters to a numeric code (0-65) */
int codonnum(char *key) {
	int i;
	for (i = 0; i < 65; i++) {
		if(strncmp(key, codoncvs[i], 3) == 0)
        	return i;
    }
    return 65;
}
...
{
    int i, j, ii, iter, jj;
    double a, b, c, delta, dlx, dw, shr, sum, thrlasso, tmp, wd[MAXSEQLEN*65], wxj[MAXSEQLEN*65];
...
int             main(int argc, char **argv)
{
    ...
    seqlen = (strlen(seq)-1)/3;
    //printf("protein length is %d.\n", seqlen);
    ...
    for (i=1; i<nseqs; i++)
    {
		...
		//    aln[i][j] = aanum(seq[j]);
		for (j=0; j<seqlen; j++) {
			aln[i][j] = codonnum(&seq[j*3]);
			//printf("%d ", codonnum(&seq[j*3]));
		}
		//printf("\n");
    }
    //return 0;
...
#pragma omp critical 
    ...
    pa = allocmat(seqlen, 65, sizeof(double));
...
#pragma omp parallel for default(shared) private(j,k,a,b)
	...
	for (j=i; j<seqlen; j++)
	{
		...
		for (a=0; a<65; a++)
		    for (b=0; b<65; b++)
				pab[a][b] /= pseudoc * 65.0 + wtsum;
	    }
	    
	    for (a=0; a<65; a++)
			for (b=0; b<65; b++)
			    if (i != j || a == b)
				cmat[i*65+a][j*65+b] = cmat[j*65+b][i*65+a] = pab[a][b] - pa[i][a] * pa[j][b];
	}
...
#pragma omp parallel for default(shared) private(j,a,b)
		    for (i=0; i<seqlen; i++)
				for (j=0; j<seqlen; j++)
				    for (a=0; a<65; a++)
						for (b=0; b<65; b++)
						    if (i != j)
								cmat[i*65+a][j*65+b] *= 1.0 - lambda;
						    else if (a == b)
								cmat[i*65+a][j*65+b] = smean * lambda + (1.0 - lambda) * cmat[i*65+a][j*65+b];
		}
    }
	...
	for (i=0; i<seqlen; i++)
		...
		for (a=0; a<65; a++)
		    for (b=0; b<65; b++)
			if ((a != b && i == j) || pa[i][64] > maxgapf || pa[j][64] > maxgapf)
			    rho[i*65+a][j*65+b] = BIG;
	...
	if (blockfn != NULL)
	{
		...
		for (a=0; a<65; a++)
		    for (b=0; b<65; b++)
		    {
			rho[(i-1)*65+a][(j-1)*65+b] = score;
			rho[(j-1)*65+b][(i-1)*65+a] = score;
		    }
	    }
